...
LSSTATUS CAgent:: LSXGetProp(PLSADTINSTDESC pInstDesc, PLSADTMSGPROP param)
{
...
LSSTATUS CAgent:: LSXSetProp(PLSADTINSTDESC pInstDesc, PLSADTMSGPROP param)
{
   LSSTATUS     stat = LSX_OK;
   LSADTPROPID  id   = param->idProp;
 ...
 *
 *******************************************************************************/
LSSTATUS CAgent:: LSXDispatchMethod(PLSADTMSGMETHOD args)
{
   LSSTATUS stat = LSX_OK;
   ...
   switch (args->idMeth)
   {
   case CCHRYSALIS_CAGENTMETHOD_BUILDAGENT:
   ...
   case CCHRYSALIS_CAGENTMETHOD_READAGENT:
      { //Force a new scope so we can repeat variable names in each case
      ...
      NotesAgent& nAgent =  (NotesAgent&) ArgList.getArg(1).getForeignObject();

      
      //Execute the method
      NotesDocument RetVal = ReadAgent(nAgent);
   ...
   default:
      ...
      ** your own classes, you can code the call thus:
         ...
         stat = CAgent::LSXDispatchMethod(args);
